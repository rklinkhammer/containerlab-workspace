...
test('capacity maximum: injected process faults retain partial evidence and enforce global limits',async()=>{
 assert.equal(process.env.CLAB_CAPACITY_FAULTS,'1');const s=JSON.parse(readFileSync(process.env.CLAB_OBSERVATION_SESSION!,'utf8'));assert.equal(s.profile,'CAPACITY-MAX');const dir=(process.env.CLAB_CAPACITY_EVIDENCE_DIR??'experiments/EXP-023-capacity')+'/faults-'+Date.now();mkdirSync(dir);const evidence:any[]=[];
 const guest=(...a:string[])=>execFileSync('limactl',['shell',s.vm,'sudo',...a],{timeout:20000,stdio:'pipe',encoding:'utf8'});const mode=(value:string)=>{guest('python3','-c','import json;json.dump({"mode":'+JSON.stringify(value)+'},open("/run/exp023-fault.json","w"))');};
 ...
 execFileSync('limactl',['copy','experiments/EXP-023-capacity/fault_cli.py',s.vm+':/tmp/exp023-fault-cli.py']);guest('mv','/usr/local/bin/containerlab','/usr/local/bin/containerlab-exp023-original');
 try{
  guest('install','-m','755','/tmp/exp023-fault-cli.py','/usr/local/bin/containerlab');mode('none');assert.ok((await snap('healthy')).endpoints.every((e:any)=>e.status==='observed'));
