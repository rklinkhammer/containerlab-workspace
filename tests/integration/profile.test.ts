...
test('actual SRL aliases, transitions, failures and replacement preserve identity boundaries',async()=>{
 assert.ok(process.env.CLAB_OBSERVATION_SESSION);const s=JSON.parse(readFileSync(process.env.CLAB_OBSERVATION_SESSION!,'utf8'));assert.equal(s.profile,'SRL-PAIR');const dir=(process.env.CLAB_EVIDENCE_DIR??'experiments/EXP-021-srl-profile')+'/attempt-'+Date.now();mkdirSync(dir);const evidence:any[]=[];
