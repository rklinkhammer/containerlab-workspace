...
test('F1 graph, parallel occurrences and keyboard-accessible inspectors',async({page})=>{
 const errors:string[]=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('/?view=evidence');
...
test('special endpoints preserve roles; duplicate occurrences remain rejected',async({page})=>{
 await page.goto('/?view=evidence');await page.getByRole('button',{name:/Special endpoints/}).click();await expect(page.locator('.react-flow__node')).toHaveCount(3);
...
test('rejection and missing-input views never fabricate graph nodes',async({page})=>{
 await page.goto('/?view=evidence');for(const scenario of ['Restricted input','Missing dependency']){await page.getByRole('button',{name:new RegExp(scenario)}).click();await expect(page.locator('.react-flow__node')).toHaveCount(0);await expect(page.getByRole('region',{name:'Diagnostics'})).toBeVisible();}
...
test('hostile display text stays inert; CSP blocks inline scripts and network',async({page,baseURL})=>{
 ...
 const response=await page.goto('/?view=evidence');expect(response!.headers()['content-security-policy']).toContain("connect-src 'self'");
...
test('bounded long text has explicit truncation; over-limit graph rejected',async({page})=>{
 await page.goto('/?view=evidence');await page.getByRole('button',{name:/Long display text/}).click();await expect(page.locator('.device-label')).toContainText('[truncated]');
...
test('recorded native single-ended link has no fabricated peer and keyboard inspector',async({page})=>{
 await page.goto('/?view=evidence');await page.getByLabel('Recorded native fixture').selectOption('N-F7');
...
test('native preview preserves parallel links, aliases, source roles and rejected inputs',async({page})=>{
 await page.goto('/?view=evidence');const picker=page.getByLabel('Recorded native fixture');await picker.selectOption('N-F1');
 await expect(page.locator('.react-flow__node')).toHaveCount(4);await expect(page.locator('.react-flow__edge')).toHaveCount(3);
...
test('recorded native evidence remains readable at narrow width',async({page})=>{
 await page.setViewportSize({width:390,height:844});await page.goto('/?view=evidence');await page.getByLabel('Recorded native fixture').selectOption('N-F7');
