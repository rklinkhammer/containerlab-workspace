...
test('fresh manifest validates graph binding; old and tampered manifests rejected before any runtime call',async()=>{
 ...
 try{
  writeFileSync(file,JSON.stringify(s));assert.equal(readObservationSession(file).binding.plan.version,'enrollment/0.2');
  ...
  for(const edit of [(x:any)=>x.binding.sourceSha256='b'.repeat(64),(x:any)=>x.profile='SRL-PAIR',(x:any)=>x.expiresAt='2000-01-01']){const x=structuredClone(s);edit(x);writeFileSync(file,JSON.stringify(x));await assert.rejects(()=>observe(),/OBSERVATION_UNAVAILABLE/);}
