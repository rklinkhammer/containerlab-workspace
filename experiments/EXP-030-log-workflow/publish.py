...
r=Path(__file__).resolve().parents[2];exp=r/'experiments/EXP-030-log-workflow';stage=exp/'publication-stage';a=r/'artifacts/design/history/A25-before-A26';sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
...
old=json.loads((r/'artifacts/design/COMPLETION.json').read_text());assert old['revision']=='A25'
assert sha(r/'artifacts/design/COMPLETION.json')==before['artifacts/design/COMPLETION.json']['sha256']
changed={'apps/web/src/workbench/Workbench.tsx','apps/web/src/workbench/NodeLogs.tsx','tests/browser/node-logs.spec.ts','tests/node-logs.test.ts'}
for e in old['files']:
...
impl=r/'artifacts/implementation/A26'
...
(impl/'PUBLICATION.json').write_text(json.dumps({'revision':'A26','archived_files':len(before),'staged_replacements':len(rows),'whole_set_atomic':False,'method':'verified full snapshot, prior-hash guards, per-file replace, completion last'},indent=2)+'\n')
paths={e['path'] for e in old['files']}|changed|{row['path'] for row in rows}|{'tests/browser/log-workflow.spec.ts','apps/web/src/workbench/log-view.ts'}
...
completion={'revision':'A26','status':'gui_phase3_bounded_log_workflow','published_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'predecessor_snapshot':str((a/'MANIFEST.json').relative_to(r)),'predecessor_manifest_sha256':sha(a/'MANIFEST.json'),'file_paths_relative_to':'workspace root','files':files}
...
print('Published and verified A26:',len(files),'files; snapshot:',len(before),'files')
