...
R=pathlib.Path('/tmp/exp012');os.chdir(R)
assert pathlib.Path('/var/run/docker.sock').exists()
assert subprocess.check_output(['sudo','docker','ps','-aq'],text=True).strip()==''
(R/'outside.txt').write_text('SYNTHETIC_OUTSIDE_012')
if not (R/'input'/'escape').is_symlink():(R/'input'/'escape').symlink_to('/tmp/exp012/outside.txt')
...
subprocess.run(['sudo','mount','-t','tmpfs','-o','size=64m,mode=1777','exp012-scratch',str(R/'scratch')],check=True)
...
r={'uid':os.getuid(),'outside_hidden':not pathlib.Path('/tmp/exp012/outside.txt').exists(),'socket_hidden':not pathlib.Path('/var/run/docker.sock').exists(),'symlink_target_hidden':not pathlib.Path('/input/escape').exists()}
