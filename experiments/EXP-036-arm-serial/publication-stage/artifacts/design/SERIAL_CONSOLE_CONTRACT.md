...
Experimental fixed-image backing classification passed actual ARM guest boot and monitor/listener negatives. This is not the proposed API: no enrolled opaque console IDs, public endpoint, host-bound cancellation, runtime replacement/expiry suite or transport was implemented. Native generic_vm lifecycle is blocked by the pinned ARM virtualization check; private-PID workaround failed namespace resolution. Keep UI unchecked/Connect disabled. See A32 results and SERIAL_PLATFORM_PROFILE.json.
