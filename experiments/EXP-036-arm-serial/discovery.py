...
def classify(expected, before, after, argv, inventory, checked_at, now, complete=True):
    ...
    backends=[argv[i+1] for i,x in enumerate(argv[:-1]) if x=='-chardev']
    if backends!=['socket,id=serial0,path=/run/appliance/serial.sock,server=on,wait=off,logfile=/run/appliance/serial.log']:
        return {'status':'unsupported','reason':'CHARDEV_PROFILE_UNQUALIFIED','consoles':[]}
    rows=[x for x in inventory['chardev'] if x['label']=='serial0']
    if len(rows)!=1 or rows[0]['filename'] not in {'unix:/run/appliance/serial.sock,server=on','disconnected:unix:/run/appliance/serial.sock,server=on'} or rows[0].get('frontend-open') is not True:
