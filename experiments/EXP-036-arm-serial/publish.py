...
r=Path(__file__).resolve().parents[2];exp=r/'experiments/EXP-036-arm-serial';stage=exp/'publication-stage';a=r/'artifacts/design/history/A31-before-A32';sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
...
old=json.loads((r/'artifacts/design/COMPLETION.json').read_text());assert old['revision']=='A31'
...
impl=r/'artifacts/implementation/A32'
...
(impl/'PUBLICATION.json').write_text(json.dumps({'revision':'A32','archived_files':len(before),'staged_replacements':len(rows),'whole_set_atomic':False,'method':'verified full snapshot, prior-hash guards, per-file replace, completion last'},indent=2)+'\n')
...
completion={'revision':'A32','status':'phase6_arm_boot_native_blocked','published_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'predecessor_snapshot':str((a/'MANIFEST.json').relative_to(r)),'predecessor_manifest_sha256':sha(a/'MANIFEST.json'),'file_paths_relative_to':'workspace root','files':files}
...
print('Published and verified A32:',len(files),'files; snapshot:',len(before),'files')
