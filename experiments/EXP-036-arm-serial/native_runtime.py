...
image=json.loads((p/'image.json').read_text())['id'];name='clab-exp036-serial-native-guest'
...
cli=['limactl','shell',v,'sudo','docker','run','--rm','--privileged','--network','host','--memory','256m','--pids-limit','128','-v','/var/run/docker.sock:/var/run/docker.sock','-v','/var/run/netns:/var/run/netns','-v','/tmp/exp036:/tmp/exp036','-w','/tmp/exp036','--entrypoint','/tmp/exp036/containerlab',image]
result={'cliProfile':'containerized-private-pid','image':image,'boot':'NOT_RUN','discovery':'NOT_RUN'};cid=None
try:
 subprocess.run(['limactl','copy',str(p/'native.clab.yml'),v+':/tmp/exp036/native.clab.yml'],check=True,timeout=20)
 with (p/'native-deploy.log').open('w') as f:
  subprocess.run(cli+['deploy','-t','/tmp/exp036/native.clab.yml'],stdout=f,stderr=subprocess.STDOUT,check=True,timeout=90)
 cid=json.loads(docker('inspect',name))[0]['Id'];result['containerId']=cid
 labels=json.loads(docker('inspect',cid))[0]['Config']['Labels'];assert labels['clab-node-kind']=='generic_vm';result['nativeKind']=labels['clab-node-kind']
 ...
 (p/('native-runtime-'+str(int(time.time()))+'.json')).write_text(json.dumps(result,indent=2)+'\n')
 # Deployment may leave a container even when deploy returns failure.
 ...
 (p/'native-cleanup.json').write_text(json.dumps({'removedContainerId':cid,'remaining':docker('ps','-aq','--filter','name=^/'+name+'$').strip()},indent=2)+'\n')
