...
set -eu
umask 077
mkdir -p /tmp/exp013/source /tmp/exp013/bin
cd /tmp/exp013
...
mkdir -p source/exp013
cp main.go source/exp013/main.go
cd source
export PATH=/tmp/exp013/go/bin:$PATH
export GOTOOLCHAIN=local
go version
timeout -k 5 900 go build -mod=readonly -o /tmp/exp013/bin/probe ./exp013
timeout -k 5 900 go build -mod=readonly -o /tmp/exp013/bin/containerlab .
sha256sum /tmp/exp013/bin/*
