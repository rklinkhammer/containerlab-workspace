...
R=pathlib.Path('/tmp/exp013');os.chdir(R)
assert pathlib.Path('/var/run/docker.sock').exists()
assert subprocess.check_output(['sudo','docker','ps','-aq'],text=True).strip()==''
(R/'outside.txt').write_text('SYNTHETIC_OUTSIDE_013')
if not (R/'input'/'escape').is_symlink():(R/'input'/'escape').symlink_to('/tmp/exp013/outside.txt')
...
subprocess.run(['sudo','mount','-t','tmpfs','-o','size=64m,mode=1777','exp013-scratch',str(R/'scratch')],check=True)
...
def run(name,args,shim=False):
 ...
 results.append(row)
try:
 for fixture in sorted(x.stem.replace('.clab','') for x in (R/'input').glob('C*.clab.yml')):
  run(fixture+'-coverage',['/probe/probe','/input/'+fixture+'.clab.yml'],True)
 ...
 # Fixed real version snapshot: no worker request can reach the actual daemon.
 for fixture in ['F1','F2','F3','F4','F5','F7','F1']:
  run(fixture+'-snapshot',['/probe/probe','/input/'+fixture+'.clab.yml'],True)
  run(fixture+'-validate-snapshot',['/probe/containerlab','--topo','/input/'+fixture+'.clab.yml','validate'],True)
 code="""import pathlib,socket,os,json
r={'uid':os.getuid(),'outside_hidden':not pathlib.Path('/tmp/exp013/outside.txt').exists(),'socket_hidden':not pathlib.Path('/var/run/docker.sock').exists(),'symlink_target_hidden':not pathlib.Path('/input/escape').exists()}
