...
exp=root/'experiments/EXP-026-four-radio';stage=exp/'publication-stage';archive=root/'artifacts/design/history/A20-before-A21';manifest=json.loads((archive/'MANIFEST.json').read_text());before={e['source_path']:e for e in manifest['files']}
...
implementation=root/'artifacts/implementation/A21';implementation.mkdir(exist_ok=True)
...
for p in sorted((root/'fixtures/bundles/FOUR-RADIO-SDR').rglob('*')):
 if p.is_file():sources.append({'path':str(p.relative_to(root)),'sha256':digest(p),'bytes':p.stat().st_size})
for name in ['package.json','package-lock.json','fixtures/bundles/catalog.json']:
...
(implementation/'PUBLICATION.json').write_text(json.dumps({'revision':'A21','archive':'artifacts/design/history/A20-before-A21/MANIFEST.json','archiveVerifiedFiles':len(before),'stagedReplacements':len(rows),'wholeSetAtomic':False,'method':'prior-hash guarded staged replacements; completion last'},indent=2)+'\n')
old=json.loads((archive/next(e['archive_name'] for e in before.values() if e['source_path']=='artifacts/design/COMPLETION.json')).read_text())
...
completion={'revision':'A21','status':'document_set_complete_four_radio_network_workflow_recorded','published_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'predecessor_snapshot':'history/A20-before-A21/MANIFEST.json','predecessor_manifest_sha256':digest(archive/'MANIFEST.json'),'file_paths_relative_to':'workspace root','files':files}
...
print('Published A21; verified',len(files),'files and predecessor')
