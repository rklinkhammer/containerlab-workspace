...
r=Path(__file__).resolve().parents[2];exp=r/'experiments/EXP-033-reanalysis';stage=exp/'publication-stage';a=r/'artifacts/design/history/A28-before-A29';sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
...
old=json.loads((r/'artifacts/design/COMPLETION.json').read_text());assert old['revision']=='A28'
...
impl=r/'artifacts/implementation/A29'
...
(impl/'PUBLICATION.json').write_text(json.dumps({'revision':'A29','archived_files':len(before),'staged_replacements':len(rows),'whole_set_atomic':False,'method':'verified full snapshot, prior-hash guards, per-file replace, completion last'},indent=2)+'\n')
paths={e['path'] for e in old['files']}|changed|{row['path'] for row in rows}|{'apps/web/src/workbench/Reanalysis.tsx','native/analysis/reanalyze.py','tests/reanalysis.test.ts'}
...
completion={'revision':'A29','status':'managed_reanalysis_selected_phase4','published_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'predecessor_snapshot':str((a/'MANIFEST.json').relative_to(r)),'predecessor_manifest_sha256':sha(a/'MANIFEST.json'),'file_paths_relative_to':'workspace root','files':files}
...
print('Published and verified A29:',len(files),'files; snapshot:',len(before),'files')
