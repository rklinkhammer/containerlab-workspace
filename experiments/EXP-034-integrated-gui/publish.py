...
r=Path(__file__).resolve().parents[2];exp=r/'experiments/EXP-034-integrated-gui';stage=exp/'publication-stage';a=r/'artifacts/design/history/A29-before-A30';sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
...
old=json.loads((r/'artifacts/design/COMPLETION.json').read_text());assert old['revision']=='A29'
...
impl=r/'artifacts/implementation/A30'
...
(impl/'PUBLICATION.json').write_text(json.dumps({'revision':'A30','archived_files':len(before),'staged_replacements':len(rows),'whole_set_atomic':False,'method':'verified full snapshot, prior-hash guards, per-file replace, completion last'},indent=2)+'\n')
paths={e['path'] for e in old['files']}|changed|{row['path'] for row in rows}|{'tests/browser/integrated-live.spec.ts'}
...
completion={'revision':'A30','status':'integrated_gui_selected_phase5','published_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'predecessor_snapshot':str((a/'MANIFEST.json').relative_to(r)),'predecessor_manifest_sha256':sha(a/'MANIFEST.json'),'file_paths_relative_to':'workspace root','files':files}
...
print('Published and verified A30:',len(files),'files; snapshot:',len(before),'files')
