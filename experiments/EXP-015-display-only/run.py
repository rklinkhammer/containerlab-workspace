...
R=pathlib.Path('/tmp/exp015');os.chdir(R)
assert pathlib.Path('/var/run/docker.sock').exists()
assert subprocess.check_output(['sudo','docker','ps','-aq'],text=True).strip()==''
(R/'outside.txt').write_text('SYNTHETIC_OUTSIDE_015')
if not (R/'input'/'escape').is_symlink():(R/'input'/'escape').symlink_to('/tmp/exp015/outside.txt')
...
subprocess.run(['sudo','mount','-t','tmpfs','-o','size=64m,mode=1777','exp015-scratch',str(R/'scratch')],check=True)
...
try:
 ...
 for fixture in manifest:
  ...
  run(fixture['id']+'-declarations',['/probe/probe','/input/'+fixture['file'],'declarations'],False)
  run(fixture['id']+'-skip-binds',['/probe/probe','/input/'+fixture['file'],'skip-binds'],True)
 code="""import pathlib,socket,os,json
r={'uid':os.getuid(),'outside_hidden':not pathlib.Path('/tmp/exp015/outside.txt').exists(),'socket_hidden':not pathlib.Path('/var/run/docker.sock').exists(),'symlink_target_hidden':not pathlib.Path('/input/escape').exists()}
...
print(json.dumps(out))""".replace('VERSION',srv.negotiated)
 ...
 run('F1-expired',['/probe/probe','/input/F1.clab.yml','skip-binds'],True)
