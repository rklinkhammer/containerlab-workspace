...
r=Path(__file__).resolve().parents[2];exp=r/'experiments/EXP-029-gui-navigation';stage=exp/'publication-stage';a=r/'artifacts/design/history/A24-before-A25';sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
...
old=json.loads((r/'artifacts/design/COMPLETION.json').read_text());assert old['revision']=='A24'
assert sha(r/'artifacts/design/COMPLETION.json')==before['artifacts/design/COMPLETION.json']['sha256']
changed={'apps/web/src/workbench/Workbench.tsx','apps/web/src/generated/workbench-recordings.json','tests/browser/workbench.spec.ts'}
for e in old['files']:
...
impl=r/'artifacts/implementation/A25'
...
(impl/'PUBLICATION.json').write_text(json.dumps({'revision':'A25','archived_files':len(before),'staged_replacements':len(rows),'whole_set_atomic':False,'method':'verified full snapshot, prior-hash guards, per-file replace, completion last'},indent=2)+'\n')
paths={e['path'] for e in old['files']}|changed|{row['path'] for row in rows}|{'tests/browser/navigation.spec.ts'}
...
completion={'revision':'A25','status':'gui_phase1_reconciled_phase2_approved_bundle_navigation','published_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'predecessor_snapshot':str((a/'MANIFEST.json').relative_to(r)),'predecessor_manifest_sha256':sha(a/'MANIFEST.json'),'file_paths_relative_to':'workspace root','files':files}
...
print('Published and verified A25:',len(files),'files; snapshot:',len(before),'files')
