...
export function projectLive(record:any,result:any,jobId:string,workerSha256:string){
 ...
 const revision=hash(JSON.stringify({bundle:record.bundleSha256,nativeCommit,workerSha256,profile:'native-approved-bundle-v1'}));
 ...
 return parseLiveGraph(JSON.stringify({...body,contract:'p1a/0.4',profile:'native-approved-bundle-v1',provenance:{sourceId:record.id,sourceSha256:source.sha256,outcomeSha256:hash(JSON.stringify(body)),nativeCommit,evidence:'executed',fieldCoordinates:null,bundleId:record.id,bundleSha256:record.bundleSha256,entryFile:record.entry,fileCount:record.files.length,workerSha256,jobId,completedAt:new Date().toISOString(),cleanup:'complete'}}));
}
