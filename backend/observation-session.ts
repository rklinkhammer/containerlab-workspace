...
export function readObservationSession(path:string|undefined){
 ...
 try{
  ...
  return{captureCapability:s.captureCapability==='capture/0.1'?s.captureCapability:undefined,vm:s.vm,expiresAt:s.expiresAt,profile:s.profile,graph,binding,logCapability:s.logCapability==='node-logs/0.1'?s.logCapability:undefined};
