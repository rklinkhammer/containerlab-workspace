...
import{captureRequest,captureResult,type CaptureRequest,type CaptureResult}from'../contracts/capture.ts';
...
const identity=(s:ReturnType<typeof session>)=>JSON.stringify([s.vm,s.binding,s.expiresAt]);
...
export async function collectCapture(input:unknown,signal:AbortSignal){
 const x=captureRequest.parse(input),s=session(),target=captureTarget(s,x);if(active)throw Error('BUSY');
 ...
 try{
  const raw=await transport(s.vm,{...target,duration:x.duration,snaplen:x.snaplen,captureFilter:x.captureFilter,displayFilter:x.displayFilter},c.signal);
  ...
  const result=captureResult.parse({contract:'capture/0.1',id:x.jobId,deploymentId:x.deploymentId,endpointId:x.endpointId,filename:x.filename,sha256:createHash('sha256').update(data).digest('hex'),bytes:data.length,createdAt,expiresAt:new Date(Date.now()+300000).toISOString(),limited:raw.limited,packets:raw.packets,analysis:raw.analysis,completeness:'not_established'});
...
const codes=new Set(['CAPTURE_NOT_QUALIFIED','CAPTURE_UNSUPPORTED','CAPTURE_UNAVAILABLE','CAPTURE_FAILED','CAPTURE_TIMEOUT','INVALID_REQUEST','MALFORMED_CAPTURE','MALFORMED_ANALYSIS','ASSOCIATION_CONFLICT','OBSERVATION_UNAVAILABLE','INCOMPATIBLE_SESSION','OUTPUT_LIMIT','BUSY','CANCELLED','ARTIFACT_UNAVAILABLE']);
