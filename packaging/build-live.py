...
r=Path(__file__).resolve().parents[1];out=r/'.runtime-live-release';out.mkdir(exist_ok=True);meta={'version':'0.3.0-preview.6','packageVersion':'0.3.0'};root=out/'payload'
...
pkg=out/'Containerlab-GUI-0.3.0-preview.6-arm64.pkg';subprocess.run(['pkgbuild','--root',str(root),'--identifier','local.containerlab.gui.live','--version',meta['packageVersion'],'--install-location','/',str(pkg)],check=True)
portable=out/'Containerlab-GUI-0.3.0-preview.6-arm64.tar.gz'
...
(r/'artifacts/implementation/LIVE-002/package-build.json').write_text(json.dumps({'version':meta['version'],'files':len(rows),'artifacts':checks,'bundledNodeSha256':expected,'signing':'unsigned local preview'},indent=2)+'\n');print(json.dumps(checks,indent=2))
