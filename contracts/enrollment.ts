...
export function derivePlan(graph:LiveGraph):EnrollmentPlan{
 ...
 return planSchema.parse({version:'enrollment/0.3',bundleId:g.provenance.bundleId,bundleSha256:g.provenance.bundleSha256,sourceSha256:g.provenance.sourceSha256,nodes,links:g.links.map(l=>({linkId:l.id,occurrence:l.occurrence})),endpoints:g.links.flatMap(l=>l.endpoints.map((e,position)=>{
...
export const bindingSchema=z.strictObject({deploymentId:hash,labName:z.string().regex(/^[A-Za-z0-9][A-Za-z0-9_.-]{0,127}$/),sourceSha256:hash,plan:planSchema,nodes:z.array(z.strictObject({nodeId:id,node:text,id:hash.nullable(),namespace:hash.nullable(),endpoints:z.array(z.strictObject({endpointId:id,item:nativeItem.nullable()})).max(32)})).max(8)}).superRefine((b,c)=>{
