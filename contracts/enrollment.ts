...
export function derivePlan(graph:LiveGraph):EnrollmentPlan{
 ...
 return planSchema.parse({version:'enrollment/0.1',bundleId:g.provenance.bundleId,bundleSha256:g.provenance.bundleSha256,sourceSha256:g.provenance.sourceSha256,nodes,links:g.links.map(l=>({linkId:l.id,occurrence:l.occurrence})),endpoints:g.links.flatMap(l=>l.endpoints.map((e,position)=>{
