...
import {z} from 'zod';
import {approvedRuntimeProfiles,bindingSchema,nativeItem,enrollmentLimits,type EnrollmentBinding} from './enrollment.ts';
...
const rawSchema=z.strictObject({ok:z.literal(true),labName:z.literal('four-radio-sdr').optional(),rows:z.array(z.strictObject({node:text,id:hash,kind:text,state:states,namespace:hash.nullable(),endpoints:z.array(result).max(32)})).max(8)});
export const multiObservationSchema=z.strictObject({contract:z.enum(['observation/0.5','observation/0.6','observation/0.7','observation/0.8']),labName:z.literal('four-radio-sdr').optional(),profile:z.enum(approvedRuntimeProfiles),deploymentId:hash,sourceSha256:hash,bundleSha256:hash,nativeCommit:z.literal('5ae50094a3afd70e4e1674fe5385e64d8979da26'),sequence:z.number().int().positive(),observedAt:z.iso.datetime(),freshForMs:z.literal(15000),linkHealth:z.literal('unknown'),limits:z.strictObject({nodes:z.literal(8),links:z.literal(16),endpoints:z.literal(32),interfacesPerNode:z.literal(64),responseBytes:z.literal(262144),collectionMs:z.literal(6000)}),nodes:z.array(z.strictObject({nodeId:id,node:text,kind:text.nullable(),containerId:hash.nullable(),state:states,association:z.enum(['enrolled_full_id','not_enrolled','unsupported'])})).max(8),endpoints:z.array(z.strictObject({endpointId:id,linkId:id,position:z.number().int().min(0).max(1),nodeId:id.nullable(),node:text,kind:text.nullable(),containerId:hash.nullable(),declaredInterface:text,observedInterface:nativeItem.shape.name.nullable(),nativeAlias:text,status:z.enum(['observed','absent','unavailable','unresolved','unsupported']),reason:reasons,namespaceFingerprint:hash.nullable(),index:z.number().int().positive().nullable(),mac:nativeItem.shape.mac.nullable(),operationalState:nativeItem.shape.operationalState,administrativeState:supplement.shape.administrativeState,carrier:supplement.shape.carrier,supplementSource:supplement.shape.source,supplementReason:supplement.shape.reason,peer:z.literal('unknown'),continuity:z.literal('unknown')})).max(32)}).superRefine((g,c)=>{
 if(g.contract==='observation/0.5'&&g.profile!=='MULTI-ENDPOINT-V2'||g.contract==='observation/0.6'&&!['CAPACITY-MEDIUM','CAPACITY-MAX'].includes(g.profile))c.addIssue({code:'custom',message:'Profile version mismatch'});
 if((g.contract==='observation/0.8')!==(g.profile==='FOUR-RADIO-SDR')||(g.contract==='observation/0.8')!==(g.labName==='four-radio-sdr'))c.addIssue({code:'custom',message:'Deployment profile mismatch'});
...
function rawFor(raw:unknown,b:EnrollmentBinding){
 if(new TextEncoder().encode(JSON.stringify(raw)).length>262144)throw Error('OUTPUT_LIMIT');const r=rawSchema.parse(raw);if((b.plan.bundleId==='FOUR-RADIO-SDR')!==(r.labName==='four-radio-sdr'))throw Error('ASSOCIATION_CONFLICT');const seen=new Set();
...
export function enroll(raw:unknown,plan:EnrollmentBinding['plan'],deploymentId:string):EnrollmentBinding{
 const empty={deploymentId,sourceSha256:plan.sourceSha256,plan,nodes:[]};const r=rawFor(raw,empty);
...
export function associateMulti(raw:unknown,binding:EnrollmentBinding,sequence:number):MultiObservation{
 ...
 return parseMultiObservation({contract:b.plan.bundleId==='FOUR-RADIO-SDR'?'observation/0.8':'observation/0.7',...(b.plan.bundleId==='FOUR-RADIO-SDR'?{labName:'four-radio-sdr'}:{}),profile:b.plan.bundleId,deploymentId:b.deploymentId,sourceSha256:b.sourceSha256,bundleSha256:b.plan.bundleSha256,nativeCommit:'5ae50094a3afd70e4e1674fe5385e64d8979da26',sequence,observedAt:new Date().toISOString(),freshForMs:15000,linkHealth:'unknown',limits:enrollmentLimits,nodes,endpoints});
}
