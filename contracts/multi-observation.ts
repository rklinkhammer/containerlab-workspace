...
function rawFor(raw:unknown,b:EnrollmentBinding){
 if(new TextEncoder().encode(JSON.stringify(raw)).length>262144)throw Error('OUTPUT_LIMIT');const r=rawSchema.parse(raw);const seen=new Set();
