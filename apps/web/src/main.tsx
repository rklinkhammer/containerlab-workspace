...
import {RuntimeObservation} from './observation.tsx';
...
function App(){
  ...
  const result=useMemo(()=>{try{if(active==='on-demand'||active==='runtime')return {graph:null,error:null};return {graph:active.startsWith('D-')?parseDeclaredGraph(JSON.stringify(declaredFixtures.find(f=>f.id===active)?.graph)):active.startsWith('N-')?parseNativeGraph(JSON.stringify(nativeFixtures.find(f=>f.id===active)?.graph)):loadScenario(active),error:null};}catch(e){return {graph:null,error:e instanceof GraphError?e.message:'The synthetic scenario could not be loaded.'};}},[active]);
  const current=active==='runtime'?{title:'Runtime observations',subtitle:'Declared topology and separate native observations'}:active==='on-demand'?{title:'On-demand native loading',subtitle:'Approved bundles · isolated declaration worker'}:scenarios.find(s=>s.id===active)??nativeFixtures.find(s=>s.id===active)??{...declaredFixtures.find(s=>s.id===active)!,subtitle:'Native declarations with explicitly unresolved prerequisites'};
  const native=active.startsWith('N-');const declared=active.startsWith('D-');
  return <div className="shell"><nav className="sidebar" aria-label="Synthetic scenarios"><div className="brand"><span aria-hidden="true">◈</span> containerlab<small>TOPOLOGY WORKBENCH</small></div><div className="sidebar-label">SYNTHETIC COLLECTION</div>{scenarios.map((s,i)=><button key={s.id} aria-current={active===s.id?'page':undefined} onClick={()=>setActive(s.id)}><span className="index">{String(i+1).padStart(2,'0')}</span><span>{s.title}</span></button>)}<label className="native-picker">Recorded native fixtures<select aria-label="Recorded native fixture" value={native?active:''} onChange={e=>e.target.value&&setActive(e.target.value)}><option value="">Choose native evidence</option>{nativeFixtures.map(f=><option key={f.id} value={f.id}>{f.title} · {f.graph.resolution}</option>)}</select></label><label className="native-picker">Declared topology fixtures<select aria-label="Declared topology fixture" value={declared?active:''} onChange={e=>e.target.value&&setActive(e.target.value)}><option value="">Choose declaration evidence</option>{declaredFixtures.map(f=><option key={f.id} value={f.id}>{f.title} · {f.graph.status}</option>)}</select></label><button aria-current={active==='runtime'?'page':undefined} onClick={()=>setActive('runtime')}>Runtime observations</button><button aria-current={active==='on-demand'?'page':undefined} onClick={()=>setActive('on-demand')}>On-demand native loading</button><div className="sidebar-footer"><span className="status-dot"/> Local preview<br/><small>Recorded, declared and runtime views</small></div></nav>
    <main><header><div><span className="eyebrow">{active==='runtime'?'P1B / RUNTIME OBSERVATION':active==='on-demand'?'P1A / ON-DEMAND':declared?'P1A / DECLARED TOPOLOGY':native?'P1A / RECORDED NATIVE PREVIEW':'P1A / SYNTHETIC PREVIEW'}</span><h1>{current.title}</h1><p>{current.subtitle}</p></div><span className="readonly">Read-only topology</span></header>
      <div className="scope"><strong>{active==='runtime'?'Controlled lab observations':'Fixture data only'}</strong><span>{active==='runtime'?'Native runtime state is observed separately from declared topology. No GUI lifecycle controls.':active==='on-demand'?'Approved fixture bundles execute only when explicitly loaded. No operational capability.':declared?'Recorded EXP-015 declarations. No live loading, full resolution or deployment.':native?'Recorded EXP-013 native resolution. No live resolution or deployment. This view does not establish corpus compatibility.':'Native resolution has not run. This view does not establish corpus compatibility.'}</span></div>
      ...
      {active==='runtime'?<RuntimeObservation/>:active==='on-demand'?<OnDemand/>:result.graph ? result.graph.contract==='p1a/0.3'?<DeclaredView key={active} g={result.graph}/>:<GraphView key={active} g={result.graph}/> : <section className="error" role="alert"><h2>Preview data rejected</h2><p>{result.error}</p><p>No partial graph or raw input was displayed.</p></section>}
      <footer>Containerlab remains the topology authority.<span>{active==='runtime'?'Contract observation/0.1 · Native inspection':active==='on-demand'?'Contract p1a/0.5 · On-demand native evidence':declared?'Contract p1a/0.3 · Declared, not resolved':native?'Contract p1a/0.2 · Recorded native evidence':'Contract p1a/0.1 · Synthetic expectations'}</span></footer>
    </main></div>;
}
