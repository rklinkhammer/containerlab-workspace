...
import {parseObservation,type Observation} from '../../../contracts/observation.ts';
export function RuntimeObservation(){
 ...
 {snapshot&&<section className="runtime-snapshot" aria-label="Runtime snapshot"><p>Deployment {snapshot.deploymentId}</p><p>Last observed: {snapshot.observedAt} · {stale?'stale':'within 15-second freshness window'}{error?' · last successful observation; current state unavailable':''}</p><p>Native commit: {snapshot.nativeCommit}</p><ul>{snapshot.nodes.map(n=><li key={n.node}>{n.node}: <strong>{n.state}</strong> · {n.containerId} · enrolled full ID</li>)}</ul><p>Link health: unknown. Container state does not establish connectivity or routing.</p></section>}
 {graph&&<DeclaredView g={graph}/>}
 </section>;
}
