...
type Observation=ReturnType<typeof parseObservation>;
export function RuntimeObservation(){
 ...
 useEffect(()=>{const c=new AbortController();fetch('/api/observation/config',{signal:c.signal}).then(r=>r.json()).then(x=>{if(c.signal.aborted)return;if(x.code==='INCOMPATIBLE_SESSION')throw Error('This observation session requires fresh enrollment.');if(!x.graph)throw Error();const g=parseLiveGraph(JSON.stringify(x.graph));if(!/^[a-f0-9]{64}$/.test(x.deploymentId))throw Error();setGraph(g);setDeployment(x.deploymentId);}).catch(e=>{if(!c.signal.aborted)setError(e instanceof Error&&e.message==='This observation session requires fresh enrollment.'?e.message:'No active runtime observation session.');});return()=>{c.abort();generation.current++;current.current?.abort();};},[]);
 ...
 async function refresh(){
  ...
  try{
   ...
   if((next.contract==='observation/0.5'||next.contract==='observation/0.6'||next.contract==='observation/0.7'||next.contract==='observation/0.8'||next.contract==='observation/0.9')&&(next.bundleSha256!==graph.provenance.bundleSha256||next.nodes.length!==graph.nodes.length||next.nodes.some(n=>!graph.nodes.some(p=>p.id===n.nodeId&&p.name===n.node&&p.kind===n.kind))||next.endpoints.length!==graph.links.reduce((n,l)=>n+l.endpoints.length,0)||next.endpoints.some(e=>{const l=graph.links.find(l=>l.id===e.linkId),p=l?.endpoints[e.position];return !p||p.nodeId!==e.nodeId||p.nodeLabel!==e.node||p.interface!==e.declaredInterface;})))throw Error('Observation identity mismatch.');
   // Sequence is local to a backend process; request tickets guard superseded replies.
   const observed=Date.parse(next.observedAt);if(observed<latest.current)return;latest.current=observed;setSnapshot(next);setNow(Date.now());
  }catch(e){if(ticket===generation.current&&!c.signal.aborted)setError(e instanceof Error&&['This observation session requires fresh enrollment.','Runtime identity conflict. Replacement resources were not associated.','Inspection failed. Absence is not established.','Observation session unavailable.','Inspection already running.','Refresh rate limited.','Inspection timed out.','Inspection output limit exceeded.','Observation unavailable.','Observation identity mismatch.'].includes(e.message)?e.message:'Observation unavailable.');}
