...
export function RuntimeObservation(){
 ...
 async function refresh(){
  ...
  try{
   ...
   if((next.contract==='observation/0.5'||next.contract==='observation/0.6'||next.contract==='observation/0.7')&&(next.bundleSha256!==graph.provenance.bundleSha256||next.nodes.length!==graph.nodes.length||next.nodes.some(n=>!graph.nodes.some(p=>p.id===n.nodeId&&p.name===n.node&&p.kind===n.kind))||next.endpoints.length!==graph.links.reduce((n,l)=>n+l.endpoints.length,0)||next.endpoints.some(e=>{const l=graph.links.find(l=>l.id===e.linkId),p=l?.endpoints[e.position];return !p||p.nodeId!==e.nodeId||p.nodeLabel!==e.node||p.interface!==e.declaredInterface;})))throw Error('Observation identity mismatch.');
