...
const blank=():Draft=>({endpoint:'',file:'',captureFilter:'',displayFilter:'',luaEnabled:false,luaFile:'',luaArgs:'',duration:'60',size:'16',snaplen:'65535'});
export function CaptureSettings({link,visible}:{link:LiveGraph['links'][number]|undefined;visible:boolean}){
 ...
 <p className="wb-small">Draft settings for this link occurrence. Retained while this topology is open; not saved to disk or sent to a worker.</p>
 ...
 <p className="wb-small">Declared endpoints only. Runtime capture support has not been qualified.</p>
 ...
 <p className="wb-small">Filename within a future managed capture directory; no host path or file is opened.</p>
 ...
 <fieldset><legend>Capture limits</legend><label>Duration (seconds)<input type="number" min="1" max="300" value={d.duration} onChange={e=>change('duration',e.target.value)}/></label><label>File limit (MiB)<input type="number" min="1" max="64" value={d.size} onChange={e=>change('size',e.target.value)}/></label><label>Snapshot length (bytes)<input type="number" min="64" max="262144" value={d.snaplen} onChange={e=>change('snaplen',e.target.value)}/></label></fieldset>
 ...
 <p className="wb-help">Filter syntax is not validated in this preview. Limits are proposed UI settings, not enforced capture controls. Capture storage, TShark and Lua execution are not implemented.</p>
 <button disabled>Start capture — unavailable</button><button onClick={()=>setDrafts(old=>({...old,[link.id]:blank()}))}>Reset capture draft</button>
 </section>;
}
