...
export function CaptureSettings({link,visible,deploymentId,onInvalidated}:{deploymentId:string|null;onInvalidated:(s:string)=>void;link:LiveGraph['links'][number]|undefined;visible:boolean}){
 ...
 <fieldset><legend>Lua filters / dissectors</legend><label><input type="checkbox" checked={d.luaEnabled} onChange={e=>change('luaEnabled',e.target.checked)}/> Include Lua settings</label>{d.luaEnabled&&<><label>Lua script reference<input value={d.luaFile} maxLength={128} placeholder="clab-probe-v1" onChange={e=>change('luaFile',e.target.value)}/></label><label>Lua script arguments<textarea value={d.luaArgs} maxLength={1024} rows={3} placeholder="One argument per line" onChange={e=>change('luaArgs',e.target.value)}/></label><p className="wb-small">Reviewed ID: clab-probe-v1 (synthetic UDP/49321). Fields: clabprobe.version, clabprobe.sequence. No script arguments are supported. Arbitrary paths and scripts are refused.</p></>}</fieldset>
 <p className="wb-help">BPF is validated by the capture tool; the display filter is applied by sandboxed TShark to a new capture or to the current managed PCAP when Reanalyze is selected. Invalid analysis does not discard a valid PCAP. Lua dissection supplies fields for display filtering; it is not a capture filter. Execution requires a qualified reviewed-script session.</p>
 <CaptureRun key={`${deploymentId}:${link.id}:${d.endpoint}:${visible}`} deploymentId={visible?deploymentId:null} endpointId={d.endpoint===''?'':`${link.id}:endpoint:${d.endpoint}`} filename={d.file} duration={d.duration} snaplen={d.snaplen} captureFilter={d.captureFilter} displayFilter={d.displayFilter} luaEnabled={d.luaEnabled} luaId={d.luaFile} luaArgs={d.luaArgs} onInvalidated={onInvalidated}/><button onClick={()=>setDrafts(old=>({...old,[link.id]:blank()}))}>Reset capture draft</button>
 </section>;
}
