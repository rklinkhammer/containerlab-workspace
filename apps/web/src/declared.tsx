...
export function DeclaredView({g}:{g:DeclaredGraph|LiveGraph}){
 ...
 return <section aria-label="Declared topology preview">
  <dl className="provenance"><dt>Source YAML</dt><dd>{g.contract==='p1a/0.5'?g.provenance.entryFile:g.provenance.sourceId+'.clab.yml'}</dd><dt>Source SHA-256</dt><dd>{g.provenance.sourceSha256}</dd><dt>Evidence</dt><dd>{g.contract==='p1a/0.5'?'On-demand native execution · '+g.provenance.completedAt:'EXP-015 · recorded declarations · no runtime socket'}</dd>{g.contract==='p1a/0.5'&&<><dt>Native commit</dt><dd>{g.provenance.nativeCommit}</dd><dt>Job / cleanup</dt><dd>{g.provenance.jobId} · {g.provenance.cleanup}</dd><dt>Bundle SHA-256</dt><dd>{g.provenance.bundleSha256}</dd></>}</dl>
  <div className="scope"><strong>{g.status==='rejected'?'Native declaration loading rejected':'Declared topology only'}</strong><span>External prerequisites remain unverified. Aliases and field provenance are unresolved. Declaration loading does not assess deployment readiness.</span></div>
  <div className="workspace"><section className="graph-column">
   ...
   <button onClick={()=>setSelected(null)}>Show all dependencies</button><h3>Dependencies ({dependencies.length})</h3><p>Partial inventory · reviewed file checks concern this bundle only, not deployment readiness. Other prerequisites are not checked. Source paths and URLs are withheld.</p><ul aria-label="Unresolved dependency list">{dependencies.map(d=><li key={d.id}><strong>{d.label}</strong> · {'availability' in d?d.availability.replaceAll('_',' '):'unresolved'} <small>({d.ownerType}: {g.nodes.find(n=>n.id===d.ownerId)?.name??`link ${(g.links.find(l=>l.id===d.ownerId)?.occurrence??0)+1}`})</small></li>)}</ul>{!dependencies.length&&<p>No listed dependencies for this selection; inventory remains partial.</p>}
