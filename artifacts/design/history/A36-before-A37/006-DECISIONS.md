...
Define and qualify one bounded read-only SDR application-health signal from the actual VRT application contract, beginning with radio readiness/control status. Specify unavailable/stale/failed states and independent expected transitions; do not infer streaming, loss-free processing, detections or recorder completeness from containers/interfaces. Keep each later pipeline capability separately gated.


## D-27 — Generic approved workbench and explicit operational placeholders

**Chosen:** GraphX-inspired graph/port selection and inspectors over native-derived DTOs. User approved GUI-1 revision3. Use metadata and generic bounded identity schemas, not four-radio branches. Freeze old parsers under legacy replay; current enrollment0.3/deployment0.2/session0.4/observation0.9 require new enrollment.

**Observed:**72 contract and30 browser checks pass;12 live browser checks skipped. Generic collector tests use mocks. Current live versions are unqualified. Prior live results remain historical. **Inferred:** independently specify node-log capability next, then qualify generic runtime before enabling transport.

Alternatives: hardcoded demonstration roles rejected; rebuilding native topology semantics rejected; implementing operational controls behind unqualified identities deferred. Capture/Lua/logs/serial controls remain explicitly non-operational. Risks: migration incompatibility, unknown console/capture support, incomplete accessibility and hierarchy. Reopen on new native kinds, operational capability, public/shared hosting or material layout change. Rollback source with reviewed Git history and documents via verified D-15 snapshot; never auto-migrate session identities. Maps B4/B5/Q-04/Q-05 and scoped S controls without historical gate promotion.


## D-28 — Native-identity bounded container logs

**Chosen:** fixed Docker stdout/stderr tail against frozen full ID, guarded by pinned native inventory and current source/bundle/deployment checks. Poll bounded snapshots instead of holding an unbounded stream. This requires no Containerlab topology interpretation or arbitrary exec API. Reuse native StreamLogs unchanged would omit historical tail and require new stream accounting; arbitrary file/command access is rejected for this slice.

**Observed:** EXP-027 qualifies selected Linux current enrollment/observation and node-logs0.1, including real marker, truncation, replacement refusal and browser display.75 contract,6 collector,31 offline browser tests pass;13 skipped; one separate live browser test passes. VM cleaned and stopped. **Limits:** polling can lose/repeat lines; cross-stream order, other native kinds/log drivers, lossless capture, log secrets redaction and immediate guest termination are not established.

**Recovery:** remove trusted logCapability or stop owned preview to disable logs. No persistence migration. Restore reviewed source and D-15 document snapshot; never reuse stopped trial VMs. Reopen for arbitrary sources, shared hosting, persistent retention, additional kinds/drivers or true streaming. B5/Q-05 and scoped S-01/S-02/S-05/S-07; no historical gate promotions.


## D-29 — Evidence-backed serial discovery before transport

**Documented:** EXP-028 pinned runtime and generic container interfaces do not provide a serial inventory; generic_vm documents Telnet5000 separately from shell/SSH. **Chosen/Inferred:** native identity plus an image-specific, read-only serial-backing adapter, with default unchecked/unsupported until qualified. Ports, labels, generic banners and container shells are insufficient. Do not actively probe/write to a serial console merely to discover it.
...
**Observed:** EXP-036 boots an experimental ARM Ubuntu guest using explicit KVM, validates serial backing and refuses a real monitor/ordinary listener. Fifteen unit negatives and three QMP process faults pass. **Documented:** pinned native virt/virt.go checks x86 flags on a host; the private-PID branch skips that check. **Observed:** direct CLI refuses virtualization; private-PID deployment fails namespace resolution despite creating a running node. Both failures are preserved; task-owned partial lab is destroyed.
