# A23 qualification update

**Observed:** EXP-027 qualifies current enrollment0.3/deployment0.2/session0.4/observation0.9 for a fresh two-node Linux RUNTIME-PAIR lab with two exact endpoint inventories. Missing/replacement identity refusal also passed. Other current-version profiles remain unqualified live. Optional trusted session logCapability=node-logs/0.1 enables the separate NODE_LOG_CONTRACT.md operation after explicit qualification; no observation DTO version change. A22's unqualified statement below is historical and superseded only within this tested scope.

# A22 current generic contract — offline verified, runtime unqualified

Current writers use enrollment/0.3, deployment/0.2, observation-session/0.4 and observation/0.9. General explicit bundle/lab identity replaces fixture-specific validation. Native inventory supplies exact names, kind labels and full IDs; host checks bind these to source/bundle and graph-derived enrollment. Same-name replacement remains a conflict. Bounds remain8 nodes/16 links/32 endpoint occurrences/64 interfaces per node,6s collector,9s transport,12s browser and256KiB output. Unsupported kinds/roles remain explicit.

Current runtime accepts only the new session/enrollment versions. Earlier contracts are replay-only in contracts/legacy and observation-history; sessions are not migrated in place. No live trial of these new versions occurred. Historical native evidence below must not be described as current-version qualification. Node logs, serial console and packet operations are absent from this observation contract.

## Historical contracts — superseded for current writers
