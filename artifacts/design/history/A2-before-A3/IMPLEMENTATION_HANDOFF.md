...
Prepared 2026-09-24 from architecture A2 and qualification run 03.
...
- Establish implementation-specific `AGENTS.md` before application work. Preserve the authority, safety and evidence rules below, but do not copy the investigation-only prohibition on production code as a permanent restriction on a separately authorized implementation repository.
...
2. [Architecture](../containerlab-investigation/artifacts/design/ARCHITECTURE.md), [decisions D-01–D-12](../containerlab-investigation/artifacts/design/DECISIONS.md), [traceability](../containerlab-investigation/artifacts/design/TRACEABILITY.md), and [implementation plan B1–B8](../containerlab-investigation/artifacts/design/IMPLEMENTATION_PLAN.md).
...
## Architecture updates required before adopting a baseline

**Observed document gap:** the latest `ARCHITECTURE_PROMPT.md` includes the following requirements, but the A2 deliverables have not been regenerated to incorporate them fully. Do not call A2 an approved or complete implementation specification.

1. **Sensitive configuration:** restricted original storage; redaction of derived views; explicit frontend field allowlists; secret-free logs, traces and errors; encryption in transit/at rest with key custody; retention/deletion across originals, scratch data, caches and backups; synthetic-secret acceptance tests. Preserve source semantics while restricting disclosure.
2. **Browser content trust:** topology, diagnostics, terminal output and packet dissections are untrusted. Specify safe text rendering, CSP, allowed link schemes, terminal escape/OSC/clipboard policy, bounded output/backpressure and malicious-content tests. No content-derived HTML execution or executable URLs.
3. **Coherent history:** before replacing design deliverables, archive the full current design set, exact prompt and other documents being replaced, with SHA-256 manifest. Verify and atomically publish the staged snapshot. Stage a coherent replacement set, check links/traceability and define interrupted-update recovery. Per-file replacement is not an atomic whole-set update.

Proposed next documentation step: produce a revised coherent architecture baseline in the new workspace, with source hashes and traceability to A2 and the updated prompt. Preserve the investigation unchanged. If instead asked to regenerate its deliverables in place, follow that prompt's complete archive protocol first. The fingerprints below identify source inputs; they are not a substitute for the required full archive.
...
Keep Q-gate status changes evidence-based. Link implementation work and test results to B1–B8, U-01–U-08 and D-01–D-12 rather than inventing a disconnected backlog.

## Proposed repository organization

This layout is **Inferred/proposed**, not scaffolded or a commitment to a backend language or separate services:

```text
containerlab-workspace/
  IMPLEMENTATION_HANDOFF.md
  AGENTS.md                  # Implementation-specific guidance
  docs/architecture/         # Reviewed baseline and provenance
...
First summarize inherited requirements, unresolved decisions and the architecture
updates needed for sensitive data, browser trust and coherent archival.
Propose a revised architecture baseline and the initial B1/B2/B3 work scope.
