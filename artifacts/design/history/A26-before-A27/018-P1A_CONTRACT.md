# Proposed P1a source-to-graph contract — A9

**A10 extension:** [declared preview contract](DECLARED_PREVIEW_CONTRACT.md) is a separate implemented DTO. Existing synthetic/resolved contracts below are unchanged.
...
## A8 native profile requirements

The implemented literal `synthetic-independent-v1` profile and two-endpoint link rule remain synthetic only. Native F7 requires one endpoint; do not change its source to accommodate the current schema. Introduce a deliberately versioned native contract, explicit link arity and unresolved source-token/field-origin state, preserving native occurrence identity and document/hash provenance. See [projection candidate](../../experiments/EXP-013-context-coverage/PROJECTION_CONTRACT.md). Authorization/job-owner/idempotency-owner requirements in earlier proposals are out of scope under TT-01; limits, input preservation and safe diagnostics remain.

## A9 implemented recorded-native specialization

`contracts/native-graph.ts` defines `p1a/0.2` / `native-recorded-exp013-v1`. Resolution is partial or rejected; complete source provenance is never implied. Endpoint arity is one or two; dummy requires one and node-to-node requires two. Original token and field pointer are null with explicit unresolved origin. IDs are revision-scoped; all endpoint/object/source references validate. The provenance block carries source ID/SHA-256, pinned resolver commit, metadata/outcome hashes and recorded EXP-013 attribution. A rejected graph contains only reviewed diagnostics. Operational fields are literal disabled/RECORDED_ONLY.

`contracts/project-native.ts` projects only reviewed native summary fields. The local generator hashes original input and fixed resolver/metadata profile for revision identity; output hashes are separate and generated MACs are excluded. Unknown native attributes are withheld, while unknown DTO fields reject. All existing finite DTO budgets apply. This recorded, single-file fixture specialization is not a multi-file bundle API or live resolver response.
