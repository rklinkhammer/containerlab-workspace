...
**A11 current:** the approved-bundle on-demand path below supplements recorded A10/A9 previews. It does not enable arbitrary ingestion, full resolution or operations.

**A10 current:** the declared-topology profile below supplements historical A9 resolved-record projection. Older next-step statements are superseded by the A10 scope.
...
## A10 declared-topology boundary

**Observed implementation:** pinned EXP-015 declaration summaries project to `p1a/0.3`, `native-declarations-exp015-v1`. Native getter kind values and declared interface strings do not assert validated kinds or normalized aliases. Dependency references are opaque, typed, unresolved, and inventory-partial. Browser fields exclude raw dependency paths/URLs and raw native errors. Reviewed template/schema messages retain input/line context. A dangling reference is not a node; external/single-ended occurrences remain selectable without fabricated peers.

No runtime service, source upload or native loader is connected to the browser. The next separate boundary is ephemeral native fixture-bundle loading with the same allowlist and isolated worker. See [contract](DECLARED_PREVIEW_CONTRACT.md) and [verification](../implementation/A10/RESULTS.md). Historical Q scores unchanged.

## A11 approved-bundle native loading

**Observed implementation:** the loopback Node preview exposes only catalog/load/cancel routes. It accepts approved bundle IDs, never user-provided paths, uploads or commands. During an explicitly created fresh VM session it invokes a fixed Linux supervisor using structured arguments and bounded JSON. The supervisor verifies a root-owned file inventory/hash, stages input in bounded tmpfs and invokes the pinned native declaration loader under an unprivileged, socket-free, network-isolated process boundary. Native logs are discarded; response fields and error messages are allowlisted before the browser.

`p1a/0.4` identifies new execution, bundle/source hashes, worker/native versions, correlation ID, completion time and cleanup. It shares object invariants with the recorded declaration contract but has separate provenance. Original bytes and context derivatives remain distinct; partial dependency inventory remains unresolved. CSP permits same-origin requests for the new API only in the sense that the server exposes finite routes; external connections remain blocked. Host/Origin checks constrain browser invocation without adding excluded TT-01 user authorization.

Timeout/cancel/error cleanup kills the exact cgroup and unmounts/removes scratch. Orphaned jobs after supervisor death are cleaned on the next exclusive load; VM lease bounds abandoned trials. No immediate power-loss cleanup guarantee. No daemon, runtime metadata, deployment, source persistence or database. See [on-demand contract](ON_DEMAND_CONTRACT.md), [A11 verification](../implementation/A11/RESULTS.md) and [EXP-016](../../experiments/EXP-016-on-demand/RESULTS.md). Historical Q scores unchanged.
