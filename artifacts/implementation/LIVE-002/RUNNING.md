...
Open **http://127.0.0.1:4173**. Full four-radio is deployed in the newly created user runtime `clab-app-3a2d71d640034b2d`. The package contains no topology bundles. Actual source remains `../containerlab-vrt/generated/four-radio.clab.yml` with seven explicitly included files; source is unchanged.
...
The server is running from `/Users/rklinkhammer/workspace/containerlab-releases/0.3.0-preview.6/Containerlab GUI Live.app`, using bundled Node and `/tmp` working directory. `user-server.json` records its exact PID/arguments; do not kill another process holding4173. No repository application code is imported at runtime.

After closing this server, the reproducible launch is:

```sh
APP="/Users/rklinkhammer/workspace/containerlab-releases/0.3.0-preview.6/Containerlab GUI Live.app/Contents/Resources"
