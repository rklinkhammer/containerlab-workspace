...
## Controlled runtime and interface observation — A14

```sh
npm ci
npm run test
npm run build
CLAB_SESSION_DIR=.runtime/exp019 python3 scripts/observation-session.py create
CLAB_OBSERVATION_SESSION=.runtime/exp019/observation-session.json CLAB_NATIVE_SESSION=.runtime/exp019/native-session.json npm run preview
```

Open http://127.0.0.1:4173, select **Runtime observations**, then **Refresh runtime** and a node or link. Inspect native eth1 operational state, enrolled attribute association, namespace fingerprint/index/MAC and freshness. Admin state, carrier, peer, continuity and link health are explicitly unknown. Declarations remain visible when runtime evidence is absent/unavailable. Optional polling runs five seconds after completion.
...
```sh
CLAB_OBSERVATION_SESSION=.runtime/exp019/observation-session.json CLAB_NATIVE_SESSION=.runtime/exp019/native-session.json npm run test:browser
CLAB_SESSION_DIR=.runtime/exp019 python3 scripts/observation-session.py stop
# A separate freshly created trial for the destructive interface suite:
CLAB_SESSION_DIR=.runtime/exp019 python3 scripts/observation-session.py create
CLAB_OBSERVATION_SESSION=.runtime/exp019/observation-session.json npm run test:interfaces
CLAB_SESSION_DIR=.runtime/exp019 python3 scripts/observation-session.py stop
npm run test:browser
```

Browser qualification changes interface state and stops/starts the left node. The opt-in interface suite deliberately removes/recreates the veth and lab, expects replacement refusal, and invalidates enrollment. Use a separate fresh trial for each suite, as the commands above do. To reproduce the complete integration then browser checks in one task requires explicit operator re-enrollment after destructive tests, as recorded in EXP-019; the application never auto-adopts replacements. Always clean up even if a test fails. Ordinary builds/tests without session variables create no VM. The historical `test:runtime` writes EXP-018 evidence; use `test:interfaces` for new transition evidence instead.

[A14 evidence](artifacts/implementation/A14/RESULTS.md) and [observation contract](artifacts/design/OBSERVATION_CONTRACT.md). The task lab is removed and VM stopped. The browser and declaration worker receive no daemon socket. The fixed root observer has native runtime/proc access; its read-only guarantee covers fixed application operations, not daemon-level credentials.
