...
r=Path(__file__).resolve().parents[3];exp=r/'artifacts/implementation/INSTALL-001';stage=exp/'publication-stage';a=r/'artifacts/design/history/A32-before-A33';sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
...
old=json.loads((r/'artifacts/design/COMPLETION.json').read_text());assert old['revision']=='A32'
assert sha(r/'artifacts/design/COMPLETION.json')==before['artifacts/design/COMPLETION.json']['sha256']
changed={'scripts/preview.mjs'}
for e in old['files']:
...
impl=r/'artifacts/implementation/A33'
...
(impl/'PUBLICATION.json').write_text(json.dumps({'revision':'A33','archived_files':len(before),'staged_replacements':len(rows),'whole_set_atomic':False,'method':'verified full snapshot, prior-hash guards, per-file replace, completion last'},indent=2)+'\n')
paths={e['path'] for e in old['files']}|changed|{row['path'] for row in rows}
for folder in [exp,a,impl,r/'packaging']:
...
completion={'revision':'A33','status':'installed_gui_preview_runtime_pending','published_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'predecessor_snapshot':str((a/'MANIFEST.json').relative_to(r)),'predecessor_manifest_sha256':sha(a/'MANIFEST.json'),'file_paths_relative_to':'workspace root','files':files}
...
print('Published and verified A33:',len(files),'files; snapshot:',len(before),'files')
