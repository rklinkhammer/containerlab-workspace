...
The server is running from `/Users/rklinkhammer/workspace/containerlab-releases/0.3.0-preview.10/Containerlab GUI Live.app`, using bundled Node and `/tmp` working directory. `LIVE-005/server.json` records its exact PID/arguments; do not kill another process holding4173. No repository application code is imported at runtime.

After closing this server, the reproducible launch is:

```sh
APP="/Users/rklinkhammer/workspace/containerlab-releases/0.3.0-preview.10/Containerlab GUI Live.app/Contents/Resources"
...
The current instance has a verified resume anchor and updated recovery helper. Use **Check recovery** for scoped inspection or **Resume / reconnect** to start/reconnect this owned instance. CLI `runtime status` works while stopped; `runtime start` never deploys a lab. See the release README for preparation of older installations and refusal behavior.

After a VM reboot, containers may retain their IDs but remain exited. Recovery reports that as partial. Use **Stop lab**, then **GO** to let Containerlab recreate the lab. Resume never silently starts/replaces individual containers or infers application readiness.
